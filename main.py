...
def _trade_pair(symbol: str):
    ...
    while True:
        ...
        try:
                ...
                if sig == "buy":
                    try:
                        ...
                        if "-2010" in str(e) or code == -2010:
                            notify.broadcast_operational_error(f"⚠️ {symbol}: not enough USDT to buy — skipping this signal.")
                    ...
                    try:
                        ...
                        notify.broadcast_operational_error(f"⚠️ {symbol} SELL failed: {e}")
            ...
            notify.broadcast_operational_error(f"⚠️ {symbol} bot crashed: {exc}\nRestarting in {backoff}s…")
            trader = None
            time.sleep(backoff)
